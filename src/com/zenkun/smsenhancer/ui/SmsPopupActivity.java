...
import com.zenkun.smsenhancer.controls.FragmentStatePagerAdapter;
import com.zenkun.smsenhancer.controls.QmTextWatcher;
import com.zenkun.smsenhancer.controls.SmsPopupPager;
import com.zenkun.smsenhancer.controls.SmsPopupPager.MessageCountChanged;
import com.zenkun.smsenhancer.preferences.ButtonListPreference;
import com.zenkun.smsenhancer.provider.SmsMmsMessage;
import com.zenkun.smsenhancer.provider.SmsPopupContract.QuickMessages;
import com.zenkun.smsenhancer.receiver.ClearAllReceiver;
import com.zenkun.smsenhancer.service.ReminderService;
import com.zenkun.smsenhancer.service.SmsPopupUtilsService;
import com.zenkun.smsenhancer.ui.SmsPopupFragment.SmsPopupButtonsListener;
import com.zenkun.smsenhancer.util.Eula;
import com.zenkun.smsenhancer.util.Log;
import com.zenkun.smsenhancer.util.ManageKeyguard;
import com.zenkun.smsenhancer.util.ManageKeywords;
import com.zenkun.smsenhancer.util.ManageNotification;
import com.zenkun.smsenhancer.util.ManagePreferences;
import com.zenkun.smsenhancer.util.ManageWakeLock;
import com.zenkun.smsenhancer.util.RetainFragment;
import com.zenkun.smsenhancer.util.SmsPopupUtils;
import com.zenkun.smsenhancer.util.ManageKeyguard.LaunchOnKeyguardExit;
...
public class SmsPopupActivity extends FragmentActivity implements SmsPopupButtonsListener {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        Log.e("Looking for theme  HoloEverywhere ");
        if(!SmsPopupUtils.isHoneycomb()) //we use Holoeverywhere for NON ICS devices :)
        {
        	//this.setTheme(R.style.Theme_HoloEverywhereDark);
        	Log.e("Using HoloEverywhere theme");
        }
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.popup);

        setupPreferences();
        //sms enhancer transparent window
        try{
    	    WindowManager.LayoutParams lp = this.getWindow().getAttributes();  
    	    lp.alpha=(float) (transparency*0.01);  
    	    this.getWindow().setAttributes(lp);
        }catch (Exception e) {
        	Log.v("Error in transparency: " +e.getMessage());
        	e.printStackTrace();
    	}
        setupViews();
	...
	private void setupViews() {
        ...
        pagerIndicator.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
            	smsPopupPager.setCurrentItem(position);
             //   Toast.makeText(getApplicationContext(), "Changed to page " + position, Toast.LENGTH_SHORT).show();
            }
    ...
    private class LoadUnreadMessagesAsyncTask extends AsyncTask<SmsMmsMessage,
        ...
        protected ArrayList<SmsMmsMessage> doInBackground(SmsMmsMessage... args) {
            ...
            //modify if keywords match
            ArrayList<SmsMmsMessage>  tmp = new ArrayList<SmsMmsMessage>();
            for (int i = 0; i < messages.size(); i++) {
            	SmsMmsMessage sms = messages.get(i);
            	ManagePreferences mp = new ManagePreferences(getApplicationContext(), sms.getContactId()
            			, sms.getContactLookupKey());
            	ManageKeywords mk = new ManageKeywords(messages.get(i), mp);
            	mk.ExistsKeywords();
                tmp.add(sms);
            }
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_QUICKREPLY:
            ...
            View qrLayout = factory.inflate(R.layout.message_quick_reply, null);
          ...
          //alpha only available on 11+ api
            int currentapiVersion = android.os.Build.VERSION.SDK_INT;
          //  qrLayout.getBackground().setAlpha((int) (transparency*0.01));
            if(currentapiVersion>= android.os.Build.VERSION_CODES.HONEYCOMB)
            {
            	try
            	{
            		qrLayout.setAlpha((float) (transparency*0.01));
            	}catch(Exception ex)
            	{
            		Log.v("Error transparency in quickReply\n"+ex.getMessage());
            	}
            ...
            try
            {
          	 contactPic= new BitmapDrawable(SmsPopupUtils.getPersonPhoto(getApplicationContext()
         			, message.getContactLookupUri()));
            }catch (Exception e) {
      		// TODO: handle exception
          	  e.printStackTrace();
          	  Log.e("cant load contatc picture");
          	  contactPic=getResources().getDrawable(R.drawable.ic_contact_picture);
      	}
