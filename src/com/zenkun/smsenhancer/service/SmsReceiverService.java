...
public class SmsReceiverService extends WakefulIntentService {
    ...
    private static final String ACTION_MESSAGE_RECEIVED =
            "com.zenkun.smsenhancer.MESSAGE_RECEIVED";
    ...
    private void notifyMessageReceived(SmsMmsMessage message) {
        ...
        //check if the contact has Keywords if not we check again for default keywords
        ManageKeywords kw = new ManageKeywords(message, mPrefs);
        boolean exist= kw.ExistsKeywords();
        boolean invertDatabase=false;
        message.isDefault(mPrefs.getDatabase()); //we select if keywords are deafault or contact
        if(!exist)
        {
        	message.isDefault(!mPrefs.getDatabase());
        	try
        	{
        	kw.useDatabase(!mPrefs.getDatabase()); //we inverted database to check default settings
        	exist = kw.ExistsKeywords();
        	}catch(Exception ex)
        	{
        		exist=false;
        	}
        ...
        //return back database settings
        
        message = kw.getMessage();
        if(exist)
        {
    	    notifEnabled = mPrefs.getBoolean(R.string.pref_keywordslistNotification_key
    			   , Defaults.PREFS_KEYWORDS_NOTIFICATIONS_ENABLE,ContactNotifications.KEY_KEYWORDS_NOTIF);
    	    message.KeywordsNotification(notifEnabled);
    	    Log.e("PopUp for Keywords");
    	     showPopup = mPrefs.getBoolean(R.string.pref_keywords_custommsg_popup_key, 
    	    		Defaults.PREFS_KEYWORDS_CUSTOM_MSG_POPUP_ENABLE,ContactNotifications.KEY_KEYWORDS_SHOWPOPUP);
        }
        if(invertDatabase)
        {
        	kw.useDatabase(!mPrefs.getDatabase());
        }
        ...
        Log.e("Iddle State= "+ callStateIdle);
