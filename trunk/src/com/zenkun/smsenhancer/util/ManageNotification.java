...
import com.zenkun.smsenhancer.receiver.ReminderReceiver;
import com.zenkun.smsenhancer.service.SmsMonitorService;
import com.zenkun.smsenhancer.ui.SmsPopupConfigActivity;
...
public class ManageNotification {
    ...
    public static final int[][] NOTIF_ICON_RES = {
            ...
            { R.drawable.stat_notify_sms_htc_default, R.drawable.stat_notify_sms_failed },
	...
	private static void notify(Context context, SmsMmsMessage message, int messageCount,
        ...
        if(message.MoveHtcSecure())
        {
    	    long _id =message.getMessageId();
    	    SmsPopupUtils.moveHtcSecureBox(context, _id);
        }
        
        if(SmsPopupUtils.isHoneycomb())
        { 
        	//sms enhancer show contact info ;)
            Bitmap largeIcon =SmsPopupUtils.getPersonPhoto(context
        	...
        	NotificationCompat.Builder jbNotif = new Builder(context);
        	jbNotif
        	.setSmallIcon(n.notifIcon)
            .setContentTitle(contentTitle)
            .setContentText(contentText)
            .setTicker(contentText)
            .setLargeIcon(largeIcon)
        	.setLights(n.notification.ledARGB, n.notification.ledOnMS, n.notification.ledOffMS) //setLights (int argb, int onMs, int offMs)
            .setContentIntent(notifIntent)
            .setNumber(messageCount)
            .setSound(n.notification.sound)
            .setVibrate(n.notification.vibrate)
            .setAutoCancel(true);
        	
        	if(SmsPopupUtils.isJB())
        	{
        		jbNotif.addAction(R.drawable.sms_enhancer, "test", notifIntent);
        	NotificationCompat.BigTextStyle s = new NotificationCompat.BigTextStyle(
                    jbNotif);
        	s.setSummaryText(message.getContactName());
        	s.bigText(contentText);
        	 n.notification  = s.build();
        	}else
        	{
        		
        		n.notification  = jbNotif.build();
        	}
        }
